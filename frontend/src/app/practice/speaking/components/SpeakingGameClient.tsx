...
interface SpeakingQuestion {
  ...
  lang: string;
  userAnswer?: boolean; // True if pronunciation is correct
  recognizedWord?: string; // Word recognized by speech API
}
...
export default function SpeakingGameClient() {
  // --- STATE MANAGEMENT ---
  ...
  const initializeQuestions = () => {
    ...
    const newQuestions: SpeakingQuestion[] = shuffledVocab.map((termData, index) => ({
      id: `sq-${index}`,
      term: termData.term,
      definition: termData.definition,
      correctAnswer: termData.term,
      lang: termData.lang || 'en-US',
    }));
  ...
  const handleAnswer = (questionId: string, isCorrect: boolean, recognizedWord: string) => {
  ...
  // --- MEMOIZED VALUES ---
  const answeredCount = useMemo(() => questions.filter(q => q.userAnswer !== undefined).length, [questions]);
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-2">
              ...
              <h1 className="text-xl font-semibold text-gray-900 dark:text-white">Luyện Nói Từ Vựng</h1>
            </div>
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-6 text-center">
          <h2 className="text-lg font-medium text-gray-900 dark:text-white">Nói từ vựng và kiểm tra phát âm</h2>
        </div>

        {questions.length > 0 ? (
          <div className="space-y-8">
            <div className="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-sm border border-gray-100 dark:border-gray-700">
              <h2 className="text-xl font-semibold text-gray-900 dark:text-white mb-4">Bài tập nói</h2>
